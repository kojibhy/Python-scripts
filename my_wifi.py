...
def check_args(args):
    try:
        if args.presharedkey and (len(args.presharedkey) < 8  or len(args.presharedkey) > 63):
        ...
        print(f"Error: {e}")
        sys.exit('[-] An error occurred while checking arguments.')
...
def start_extentions(wlan=None, gateway=None):
    try:
        ...
        print(f"Error: {e}")
        sys.exit('[-] An error occurred while starting extensions.')
